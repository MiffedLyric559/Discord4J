...
import discord4j.gateway.distributed.PayloadSource;
import discord4j.gateway.distributed.SourceMapper;
...
public class KafkaPayloadSource<K, V> implements PayloadSource {
    ...
    private final SourceMapper<K, V, GatewayPayload<?>> mapper;
    private final SourceMapper<K, V, NodeControl> controlMapper;

    public KafkaPayloadSource(Properties properties, String topic, SourceMapper<K, V, GatewayPayload<?>> mapper,
                              SourceMapper<K, V, NodeControl> controlMapper) {
    ...
    @Override
    public Flux<?> receiveControl(Function<NodeControl, Mono<Void>> processor) {
        ReceiverOptions<K, V> options = receiverOptions.subscription(Collections.singleton(topic))
                .addAssignListener(partitions -> log.debug("Partitions assigned: {}", partitions))
                .addRevokeListener(partitions -> log.debug("Partitions revoked: {}", partitions));
        Flux<ReceiverRecord<K, V>> kafkaFlux = KafkaReceiver.create(options).receive();
        return kafkaFlux
                ...
                .flatMap(record -> controlMapper.apply(Tuples.of(record.key(), record.value())))
                .flatMap(processor);
    }
}
